...
#part 2 Lidar data is wrong because of wrong rotation so give less-weightage(more-uncertain) to LIDAR and MORE weightage(less-uncertainty) to GNSS while calculating K (optimal gain)
var_gnss  = 0.01
var_lidar = 100.0

#part 3 no GNSS and NO Lidar for some time so give less weightage (more uncertain) for these while calculating K (optimal gain)
# var_gnss  = 1.0
# var_lidar = 10.0
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    R = sensor_var * np.eye(3) # sensor measurese position[x,y,z] there 3 degrees of uncorrelated uncertainties
    K_k_1 = np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + R)
    K_k = p_cov_check.dot(h_jac.T).dot(K_k_1)

    # 3.2 Compute error state
    delta_x_k = K_k.dot(y_k - p_check)
    ...
    #  quat_mult_left == self*q and quat_mult_right == q*self
    # q_hat = q(delta_phi) * q_check -- q(delta_phi) left multiplied to q_check
    q_hat = Quaternion(euler=delta_x_k[6:]).quat_mult_left(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    Q_k_1 = np.eye(6)
    Q_k_1[0:3, 0:3] = var_imu_f * np.eye(3)
    Q_k_1[3:6, 3:6] = var_imu_w * np.eye(3)
    Q_k_1 = (delta_t**2)*Q_k_1


    # 2. Propagate uncertainty

    p_cov_check = (F_k_1.dot(p_cov_check).dot(F_k_1.T)) + (l_jac.dot(Q_k_1).dot(l_jac.T))

    # 3. Check availability of GNSS and LIDAR measurements
    gotMesur = False
    for i in range(len(gnss.t)):
        if imu_f.t[k] == gnss.t[i]:
            gotMesur = True
            sensor_var = var_gnss
            y_k = gnss.data[i]
            break
